 ...
 *
 * Uses the Cloudflare TypeScript SDK for API operations and shells out
...
/** Read wrangler.toml as a string. */
...
function readDeployConfig(): DeployConfig | null {
  if (!existsSync(DEPLOY_TOML)) return null;
...
function writeDeployConfig(config: DeployConfig): void {
  const lines = [
    ...
    "# that get patched into wrangler.toml at deploy time.",
    "",
...
/**
 * Patch wrangler.toml with deploy-specific values from wrangler-deploy.toml.
 * Saves a backup so we can restore the clean version afterwards.
 */
function patchTomlForDeploy(): void {
  const config = readDeployConfig();
  if (!config) die("No deploy config found. Run 'install' first.");

  // Save the clean version
  const clean = readToml();
  writeFileSync(WRANGLER_TOML + ".clean", clean);
  ...
  if (config.domain) {
    ...
    toml = toml.replace(/^# DEPLOY:ROUTES$/m, routesBlock);
  }
...
/**
 * Restore wrangler.toml to the clean (git-tracked) version.
 * Called after wrangler build/deploy.
 */
function restoreCleanToml(): void {
  const cleanPath = WRANGLER_TOML + ".clean";
  if (existsSync(cleanPath)) {
    const clean = readFileSync(cleanPath, "utf-8");
    writeToml(clean);
...
async function cmdInstall(): Promise<void> {
  ...
  // 5. Write deploy config (gitignored) and patch wrangler.toml for deploy
  writeDeployConfig({ database_id: dbId, domain: customDomain });

  // 6. Build and deploy (wrangler handles bundling + upload)
  patchTomlForDeploy();
  try {
    info("Building...");
    ...
    const deployCode = await run(["bunx", "wrangler", "deploy"], { cwd: WEB_DIR });
    if (deployCode !== 0) die("Deploy failed.");
  } finally {
    restoreCleanToml();
  }
...
async function cmdUpgrade(): Promise<void> {
  ...
  if (!config) {
    die(
      "No deploy config found (apps/web/wrangler-deploy.toml).\n" +
        "Run './deploy.sh install' first.",
    );
  }
  ...
  // Build and deploy with patched wrangler.toml
  patchTomlForDeploy();
  try {
    info("Building...");
    const buildCode = await run(["bunx", "astro", "build"], { cwd: WEB_DIR });
...
async function cmdDestroy(): Promise<void> {
  ...
  // 4. Remove the deploy config file
  removeDeployConfig();
