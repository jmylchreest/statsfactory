...
export default function SessionTimeline() {
  ...
  const copySessionJson = useCallback(() => {
    if (!selectedSession || timelineEvents.length === 0) return;
    const doc = {
      session_id: selectedSession,
      app_id: appId,
      event_count: timelineEvents.length,
      first_event: timelineEvents[0].timestamp,
      last_event: timelineEvents[timelineEvents.length - 1].timestamp,
      events: timelineEvents.map((ev) => ({
        id: ev.id,
  ...
  }, [selectedSession, appId, timelineEvents]);
